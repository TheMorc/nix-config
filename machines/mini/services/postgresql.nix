{ config, ... }:
{
  ...
  system.activationScripts.postgresWorkaround = ''
    ...
    chown postgres:postgres /mini_local/postgresql/${config.services.postgresql.package.psqlSchema}
    chmod 700 /mini_local/postgresql/${config.services.postgresql.package.psqlSchema}
  '';
